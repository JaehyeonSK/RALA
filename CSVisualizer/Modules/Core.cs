...
namespace CSVisualizer.Modules
{
    ...
    {
        public static string Shorten(this Guid guid) => guid.ToString().Substring(0, 8);
    }
    ...
    public class Core
    {
        ...
        public void Start(string path)
        {
            ...
            {
                guiHandler.WriteLog("the file doesn't exist.");
                return;
            }
            ...
            #region 테스트 용
            guiHandler.WriteLog("==Using List==");
            foreach (var unit in codeUnitManager.UsingList)
            {
                guiHandler.WriteLog(unit);
            }
            guiHandler.WriteLog("====\n");

            guiHandler.WriteLog("==Declare List==");
            ...
            guiHandler.WriteLog("");
            guiHandler.WriteLog("==Execution Result==");
        ...
        private void ProcessNewMethod(FuncCallUnit funcCallUnit)
        {
            ...
            guiHandler.WriteLog(funcCallUnit.Content);
            ...
            while (execList.Count > 0)
            {
...
#if debug
                    guiHandler.WriteLog($"{varInfo.Type} {varInfo.Name} (Value: {varInfo.Value}) is created in Stack");
#endif
                }
                else if (codeUnit is VarDeclUnit) // 선언만 할 경우
                {
                    ...
                    memoryManager.CreateVariable(codeUnit.Guid, varInfo);
#if debug
                    guiHandler.WriteLog($"{varInfo.Type} {varInfo.Name} is created in Stack");
#endif
                }
...
#if debug
                            guiHandler.WriteLog($"{varInfo.Type} {varInfo.Name} <= null");
#endif
                        }
                        else
                        {
...
#if debug
                            guiHandler.WriteLog($"{varInfo.Type} {varInfo.Name} <= {(Guid)rightVar.Value}");
#endif
                        }
                    }
                    else // 값형 변수일 경우
...
#if debug
                        guiHandler.WriteLog($"{varInfo.Type} {varInfo.Name} <= {varInfo.Value}");
#endif
                    }
                }
            }
